  ...
  describe("Reports", () => {
    ...
    describe("createReport", () => {
      beforeAll(async () => {
        testReport = await createReport({
          ...
          location: "Bermuda",
          description:
            "The ship seemed to disappear as it floated through this triangle.",
    ...
    describe("getOpenReports", () => {
      beforeAll(async () => {
        ...
        [singleReport] = reports.filter(
          (report) => report.id === reportIdToCreate
        );
      });
    ...
    describe("closeReport", () => {
      ...
      it("If report doesnt exist, throws an error with a useful message", async () => {
        await expect(closeReport(300)).rejects.toThrow(
          "Report does not exist with that id"
        );
      });
      it("If the passwords dont match, throws an error", async () => {
        await expect(
          closeReport(reportIdToCreate, "iLoveNothing")
        ).rejects.toThrow(
          "Password incorrect for this report, please try again"
        );
      });
      ...
      it("If it has already been closed, throws an error with a useful message", async () => {
        await expect(
          closeReport(reportIdToCreate, "ShipIsNoMore")
        ).rejects.toThrow("This report has already been closed");
      });
    ...
    describe("createReportComment", () => {
      const commentFields = {
        content: "something strange is happening in this galaxy, for sure",
      };
      ...
      it("if it wasnt found, throw an error saying `That report does not exist, no comment has been made`", async () => {
        await expect(createReportComment(500, commentFields)).rejects.toThrow(
          "That report does not exist, no comment has been made"
        );
      });
      it("if the current date is past the expiration, throw an error saying `The discussion time on this report has expired, no comment has been made`", async () => {
        ...
        await expect(
          createReportComment(reportIdForComments, commentFields)
        ).rejects.toThrow(
      ...
      it("if it is not open, throw an error saying `That report has been closed, no comment has been made`", async () => {
        ...
        await expect(
          ...
          "That report has been closed, no comment has been made"
        );
      });
    });
  });
